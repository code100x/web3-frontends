...
Run the `code100x-ui` init command to setup your project:
...
When running `npx code100x-ui@latest init`, your tailwind config for content will be overwritten. To fix this, change the `module.exports` to `export default` and the `content` section with the code below to your `tailwind.config.mjs` file:
...
```bash
npx code100x-ui@latest add button
```
